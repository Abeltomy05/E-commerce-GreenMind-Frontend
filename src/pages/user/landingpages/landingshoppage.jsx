  ...
  const fetchProducts = async () => {
    try {
      const [productResponse, offersResponse] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL}/user/getproductdatalandingpage`),
        axios.get(`${import.meta.env.VITE_API_URL}}/user/getactiveofferslandingpage`)
      ]);
  ...
  const handleSearch = async (term) => {
    try {
      ...
      if (!term.trim()) {
        ...
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/user/searchlandingpage?query=${encodeURIComponent(term)}`);
