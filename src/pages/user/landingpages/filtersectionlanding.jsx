...
function LandingFilterSection({ isOpen, onProductsUpdate, isSearchActive  }) {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const categoryResponse = await axios.get(`${import.meta.env.VITE_API_URL}/user/getcategorynameslandingpage`);
        const categoryData = categoryResponse.data.data.map(category => category.name);
        setCategories(categoryData);

        const typesResponse = await axios.get(`${import.meta.env.VITE_API_URL}/user/producttypeslandingpage`);
      ...
      if (!isSearchActive) {
        const fetchFilteredProducts = async () => {
          try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/user/productsfilterlandingpage?${params}`);
